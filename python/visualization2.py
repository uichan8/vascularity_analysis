...
mixed_img = 0.95 * img.astype(np.float32)/255.0 + 0.05 * mask.astype(np.float32)/255.0
...
for vessel in vessel_informs:
    ...
    for i in tqdm(range(1,vessel[0])):
        ...
        edge_x, edge_x2, edge_y, edge_y2 = mask_width_detection(vessel[2],np.c_[sorted_x,sorted_y])

        if len(edge_x) > 4:
            draw_line(branch_mask, (edge_y[1],edge_x[1]), (edge_y2[1],edge_x2[1]), color = vessel[3])
            draw_line(branch_mask, (edge_y[-2],edge_x[-2]), (edge_y2[-2],edge_x2[-2]), color = vessel[3])
        ...
        for i in range(len(x_cen)):
            if mask[int(x_cen[i]),int(y_cen[i]),0] == 255 and mask[int(x_cen[i]),int(y_cen[i]),2] == 255:
                a,b = subpixel_edge.get_edge(blur_img,(x_cen[i],y_cen[i]), center_tan[i],vessel_w[i],radius_edge_width_ratio = 1/vessel_w[i], sampling_num = 50, power_factor = 2)
                subcen_x = np.append(subcen_x, (a[0]+b[0])/2)
                subcen_y = np.append(subcen_y, (a[1]+b[1])/2)
                r = np.append(r, np.sqrt((a[1] - b[1])**2+(a[0]-b[0])**2)/2)
            else:
                a,b = subpixel_edge.get_edge(blur_img,(x_cen[i],y_cen[i]), center_tan[i],vessel_w[i],radius_edge_width_ratio = 3/vessel_w[i], sampling_num = 50, power_factor = 2)
                subcen_x = np.append(subcen_x, (a[0]+b[0])/2)
                subcen_y = np.append(subcen_y, (a[1]+b[1])/2)
                r = np.append(r, np.sqrt((a[1] - b[1])**2+(a[0]-b[0])**2)/2)
    ...
    if vessel[3] == 'r':
        ...
        plt.scatter(sub_edge_y,sub_edge_x, c = 'g', s = 1)
        

plt.scatter(center_y, center_x, c = 'white', s = 1)
